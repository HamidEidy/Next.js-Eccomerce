...
const cartReducer = (state: any = initialState, action: any) => {
    switch (action.type) {
        case ADD_TO_CART:
            const hasProduct = state.cart.find(p => p.id === action.payload.product.id) ? true : false;
            state.cart = hasProduct ? state.cart.map(p => p.id === action.payload.product.id ? { ...p, qty: p.qty + 1 } : p) : [...state.cart, { ...action.payload.product, qty: action.payload.qty }];
        ...
        case INCREMENT:
            state.cart = state.cart.map((p: any) => p.id === action.payload ? { ...p, qty: p.qty + 1 } : p);
            saveStorage(state.cart)
            return {
                ...state,
                cart: state.cart
        ...
        case CLEAR_CART:
            saveStorage([])
            toast.warning("سبد خرید شما خالی شد!")
            return {
                ...state,
                cart: []
            }
